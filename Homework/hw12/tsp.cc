...
//////////////////////////////////////////////////////////////////////////////
// The original genetic algorithm search will be used as a "helper" function
// for the new implementation of a multithreaded GA search.
Cities::permutation_t
orig_ga_search(const Cities& cities,
          unsigned iters,
          unsigned pop_size,
...
{
  std::vector<std::thread> threads;
  auto best_order = random_permutation(cities.size());
  auto thread_ga = [&]() { // This lambda runs the genetic algorithm search in a thread
    auto ga_order = orig_ga_search(cities, iters / nthread, pop_size, mutation_rate);
    if (cities.total_path_distance(ga_order) < cities.total_path_distance(best_order)) {
      best_order = ga_order;
    }
  };
  for (unsigned i = 0; i < nthread; i++) { // Have each thread run the original ga_search
    threads.push_back(std::thread(thread_ga));
  }
  for (auto& t : threads) { // Recombine all the threads
    t.join();
  }
  return best_order;
}
...
int main(int argc, char** argv)
{
  ...
  const auto nthread = (argc > 4)? atoi(argv[4]) : 4;
